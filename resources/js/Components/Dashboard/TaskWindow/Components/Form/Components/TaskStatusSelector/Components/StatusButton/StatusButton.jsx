...
export const StatusButton = ({ task, statuses, onClickHandler }) => {
    ...
    return (
        <button id={"status"} onClick={onClickHandler} className={`flex items-center justify-center bg-${taskStatusColor.main} rounded-l h-6 border-r border-${taskStatusColor.contrast} text-xs uppercase text-${taskStatusColor.content} font-medium px-2`}>
