...
export default function TaskWindow({ task, statuses, formattedStatuses, formattedStatusesByType }) {
    ...
    const taskStatusColor = colors[formattedStatuses[task.status_id].color]
    ...
    const handleCompleteButtonClick = () => {
        router.put(`/tasks/${task.id}`, {
            status_id: formattedStatusesByType.closed[0].id
        })
    }
    ...
    const handleNextStatusButtonClick = () => {
        ...
        statuses.forEach((status, index) => {
            if (status.name === formattedStatuses[task.status_id].name && statuses[index + 1]) {
                router.put(`/tasks/${task.id}`, {
        ...
        if (!exit) {
            router.put(`/tasks/${task.id}`, {
                status_id: formattedStatusesByType['not started'][0].id
            })
        }
    }
                    ...
                    <div className="max-w-2xl mx-auto mt-6 flex flex-col">
                        ...
                        <div className={"mt-7 flex items-center gap-1 pl-1.5 pb-8 min-h-9"}>
                            ...
                            <div className={`flex max-w-72 py-1.5 pl-1.5 grow rounded-md ${isStatusDropdownActive ? "bg-gray-100" : ""}`}>
                                <button id={"status"} onClick={() => setIsStatusDropdownActive(true)} className={`flex items-center justify-center bg-${taskStatusColor.main} rounded-l h-6 border-r border-${taskStatusColor.contrast} text-xs uppercase text-${taskStatusColor.content} font-medium px-2`}>{formattedStatuses[task.status_id].name }</button>
                        ...
                        <div className={"flex border rounded p-3"}>
                            ...
                            {showingDescriptionInput && <textarea name="task-description" rows="10" placeholder={"Write something"} onChange={(e) => setTaskDescriptionInput(e.target.value)} value={taskDescriptionInput ? taskDescriptionInput : ''} autoFocus={!taskDescriptionInput} className={"border-0 focus:ring-0 resize-none pl-5 w-full"}></textarea>}
                        </div>
                    </div>
                </div>
            </div>
